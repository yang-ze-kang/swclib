...
from swclib.geometry.Transformations import superimposition_matrix
import numpy as np
...
class Sphere(Obj3D):
    ...
    def calBBox(self):
            ...
            min_bound = np.floor(np.min(min_mat, 0)).astype(np.int64)
            max_bound = np.ceil(np.max(max_mat, 0)).astype(np.int64)
