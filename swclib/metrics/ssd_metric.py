...
class SSDMetric:
    ...
    def run(self, gold, pred):
        ...
        mask = dists_gt2pred > self.min_distance
        if mask.sum() == 0:
            ssd_pred2gt = 0
        else:
            ssd_pred2gt = np.mean(dists_gt2pred[mask].mean())

        # pred->gold
        tree_pred = cKDTree(pred_coords)
        dists_pred2gt, _ = tree_pred.query(gt_coords, k=1)
        sd_gt2pred = np.mean(dists_pred2gt)
        mask = dists_pred2gt > self.min_distance
        if mask.sum() == 0:
            ssd_gt2pred = 0
        else:
            ssd_gt2pred = np.mean(dists_pred2gt[mask].mean())
